...
int StartsWith(const char* data, const char* toComp, char* result);
void GaukServerioVarda(const char* zinute, char* serverio_vardas, char* kirpta_zinute);
void TerpkSaltinioServerioVarda(char *kirpta_zinute, const char* vardas);
...
#endif // _WIN32
{
    ...
    APPROVE_NAME:
        send(socket_handle, "VARDASOK", strlen("VARDASOK"), 0);
        printf("Serveris %s prijungtas\n", vardas);
        ...
        while (1) {
            ...
            char zinute[512] = { 0 };
            if (StartsWith(duomenys, "TARPSERVER", zinute)) {
                 char serverio_vardas[64] = { 0 };
                 char kirpta_zinute[512] = { 0 };

                 GaukServerioVarda(zinute,  serverio_vardas,  kirpta_zinute);
                 TerpkSaltinioServerioVarda(kirpta_zinute, vardas);
                for (int i = 0; i < dabartinis_serveriu_sk; i++) {
                    if (strcmp(serverio_vardas, vardai[i]) == 0) {
                        send(soketai[i], kirpta_zinute, strlen(kirpta_zinute), 0);
                        break;
                    }
                }
            }
...
int StartsWith(const char* data, const char* toComp, char* result){
    int i;
    for (i = 0; i < strlen(toComp); i++) {
        if (data[i] != toComp[i]) {
            return 0;
        }
    }
    for (int y = 0; i < strlen(data); i++, y++) {
        result[y] = data[i];
    }
    return 1;
}
...
void GaukServerioVarda(const char* zinute, char* serverio_vardas, char* kirpta_zinute) {
    int i;
    if (zinute[0] != '@') {
        printf("Error: wrong format received\n");
        exit(5);
    }

    for (i = 1; zinute[i] != ' '; i++) {
        serverio_vardas[i - 1] = zinute[i];
    }

    serverio_vardas[i - 1] = 0;
    i++;
    int y;
    for (y = 0; i < strlen(zinute); y++, i++) {
        kirpta_zinute[y] = zinute[i];
    }
    kirpta_zinute[y] = 0;
}
...
void TerpkSaltinioServerioVarda(char* kirpta_zinute, const char* vardas) {
    int i;
    for (i = 0; kirpta_zinute[i] != ' '; i++);

    int old_i = i;
    i++;
    char remain_message[1024] = { 0 };
    for (int y = 0; i < strlen(kirpta_zinute); i++, y++) {
        remain_message[y] = kirpta_zinute[i];
    }

    i = old_i;
    kirpta_zinute[i] = '@';
    i++;
    for (int y = 0; y < strlen(vardas); y++, i++) {
        kirpta_zinute[i] = vardas[y];
    }

    for (int y = 0; y < strlen(remain_message); y++, i++) {
        kirpta_zinute[i] = remain_message[y];
    }
}
...
#endif // _WIN32
{
    ...
    APPROVE_NAME:
        ...
        while (1) {
            ...
            if (duomenys[0] == '@') {
                char message[1024] = "TARPSERVER";
                char serveris[64] = "@";
                int i;
                for (i = 0; duomenys[i] != ' '; i++) {
                    serveris[i] = duomenys[i];
                }
                i++;
                char zinutes_tekstas[512] = { 0 };
                for (int y = 0; i < strlen(duomenys); i++, y++) {
                    zinutes_tekstas[y] = duomenys[i];
                }
                strcat(message, serveris);
                strcat(message, " ");
                strcat(message, vardas);
                strcat(message, " : ");
                strcat(message, zinutes_tekstas);
                send(cloud_server_soketas, message, strlen(message), 0);
            }
            else {
...
#ifdef linux
void* CloudConnectionValdymas(void* lpArg)
#endif // linux
#ifdef _WIN32
DWORD WINAPI CloudConnectionValdymas(LPVOID lpArg)
#endif // _WIN32
{
    while (1) {
        char duomenys[512] = { 0 };
        int read_val = recv(cloud_server_soketas, &duomenys, 512, 0);
        if (read_val == -1) {
            break;
        }
        for (int i = 0; i < dabartinis_serveriu_sk; i++) {
...
int main(int argc, char* argv[]) {
    ...
    if ((status = getaddrinfo(cloudHostas, cloudHostasPortas, &hints2, &res2)) != 0) {
    ...
    cloud_server_soketas = ErrorCheck(socket(res2->ai_family, res2->ai_socktype, res2->ai_protocol), "Cloud socket problem");
    ErrorCheck(connect(cloud_server_soketas, res2->ai_addr, res2->ai_addrlen), "Cloud connection problem");

    while (1) {
        printf("Vesk serverio varda: \n");
        char serverio_vardas[64] = { 0 };
        scanf("%s", &serverio_vardas);
        send(cloud_server_soketas, serverio_vardas, strlen(serverio_vardas), 0);
        char atsakymas[64] = { 0 };
        recv(cloud_server_soketas, atsakymas, 64, 0);
        if (strcmp(atsakymas, "VARDASOK") == 0) {
            printf("Priimtas vardas: %s\n", serverio_vardas);
            break;
        }
        else {
            printf("Toks serveris jau egzistuoja\n");
        }
    }

#ifdef linux
    pthread_t tid;
    int error_val = pthread_create(&tid, NULL, CloudConnectionValdymas, NULL);
    if (error_val != NULL) {
...
#ifdef _WIN32
    HANDLE handle = CreateThread(NULL, 0, CloudConnectionValdymas, NULL, 0, NULL);
    if (handle == NULL) {
        fprintf(stderr, "Could not create Thread\n");
    }
#endif // _WIN32
    ...
    __finally {
        ...
        CONNECTION_CLOSE(cloud_server_soketas);
